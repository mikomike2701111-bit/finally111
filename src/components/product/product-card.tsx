    ...
    <div className="relative group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-md">
      <Link href={`/products/${product.slug}`} className="block">
        <div className="aspect-[3/4] relative overflow-hidden bg-gray-50">
          ...
          {/* Quick Add Overlay */}
          <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-20 bg-gradient-to-t from-black/60 to-transparent">
             <button
              onClick={handleAddToCartClick}
              className="w-full h-10 bg-white text-black rounded-full flex items-center justify-center gap-2 text-sm font-bold shadow-lg hover:bg-gray-100 active:scale-95 transition-all"
            >
              <ShoppingBag size={16} />
              Quick Add
            </button>
        ...
        <div className="p-4 space-y-1">
          <div className="flex justify-between items-start gap-2">
            <h3 className="font-medium text-gray-900 truncate text-sm flex-grow">{product.name}</h3>
            <button
